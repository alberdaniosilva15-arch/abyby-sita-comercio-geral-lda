...
export const ActivitiesVideoModal: React.FC<ActivitiesVideoModalProps> = ({ video, onClose }) => {
  const [isMuted, setIsMuted] = React.useState(true); // Sempre sem som por predefinição
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-10">
        ...
        <motion.div
          ...
          className="relative z-10 w-full max-w-4xl bg-[#071B2E] border border-cyan-500/30 rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(0,240,255,0.15)] flex flex-col md:flex-row"
        >
          {/* Botão de Fechar */}
          <button
            onClick={onClose}
          ...
          {/* Área do Vídeo */}
          <div className="relative w-full md:w-3/5 bg-black flex items-center justify-center min-h-[280px] sm:min-h-[360px] md:min-h-[440px]">
            <video
              src={video.videoUrl}
              autoPlay
              loop
            ...
            {/* Alternador de Som Rápido */}
            <button
              onClick={() => setIsMuted(!isMuted)}
              className="absolute bottom-4 left-4 z-20 px-3 py-1.5 rounded-full bg-slate-900/80 border border-white/20 text-xs font-mono text-slate-200 hover:text-cyan-300 flex items-center gap-1.5 transition-all cursor-pointer backdrop-blur-sm"
            >
              {isMuted ? (
                <>
                  <VolumeX className="w-4 h-4 text-slate-400" />
                  <span>Sem Som (Predefinido)</span>
                </>
              ) : (
                <>
          ...
          {/* Painel Informativo da Atividade ("DIZER") */}
          <div className="w-full md:w-2/5 p-6 sm:p-8 flex flex-col justify-between bg-gradient-to-b from-[#0B253E] to-[#071B2E] border-t md:border-t-0 md:border-l border-white/10">
            <div>
              ...
              <h3 className="text-xl sm:text-2xl font-display font-extrabold text-white mb-3 leading-snug text-readable-heading">
                {video.title}
              </h3>

              {/* Bloco "DIZER" / Legenda Profissional */}
              <div className="p-4 rounded-xl bg-white/[0.04] border border-white/10 mb-6">
                <div className="flex items-center gap-1.5 text-[11px] font-mono uppercase text-cyan-400 font-semibold mb-1.5">
